...
static void activateFoldover(LADSPA_Handle instance) {
...
/* Added DC offset removal filter
   Run adding version below does not have this
   Do not use run adding for this reason */
...
#define D_(s) (s)
	...
	if (foldoverDescriptor) {
		...
		foldoverDescriptor->activate = activateFoldover;
		...
		foldoverDescriptor->run_adding = NULL; /*runAddingFoldover;*/
		foldoverDescriptor->set_run_adding_gain = NULL; /*setRunAddingGainFoldover;*/
	}
}
