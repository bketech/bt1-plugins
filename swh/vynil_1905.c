...
static LADSPA_Handle instantiateVynil(
 ...
 unsigned long s_rate) {
	...
	fixp16 click_buffer_omega = {0};
	fixp16 click_buffer_pos = {0};
	float click_gain;
	float def;
	float def_target;
	float fs;
...
//#line 37 "vynil_1905.xml"
	...
	buffer_m = calloc(buffer_size, sizeof(LADSPA_Data));
	buffer_s = calloc(buffer_size, sizeof(LADSPA_Data));
	buffer_mask = buffer_size - 1;
	buffer_pos = 0;
	click_gain = 0;
	phi = 0.0f; /* Angular phase */

	click_buffer = calloc(CLICK_BUF_SIZE, sizeof(LADSPA_Data));
...
#define D_(s) (s)
	...
	if (vynilDescriptor) {
		...
		vynilDescriptor->run_adding = NULL; //runAddingVynil;
