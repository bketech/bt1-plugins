...
instantiate_Echo(const LADSPA_Descriptor * Descriptor,
	...
	if ((ptr = (void **) calloc(1, sizeof(Echo))) != NULL) {
...
activate_Echo(LADSPA_Handle Instance) {
	...
	if ((ptr->ringbuffer_R = (LADSPA_Data*)
	     calloc(MAX_DELAY * ptr->sample_rate / 1000,
		    sizeof(LADSPA_Data))) == NULL)
		exit(1);
	if ((ptr->buffer_pos_L = (long unsigned int*) calloc(1, sizeof(unsigned long))) == NULL)
		exit(1);
	if ((ptr->buffer_pos_R = (long unsigned int*) calloc(1, sizeof(unsigned long))) == NULL)
		exit(1);
