...
instantiate_ChorusFlanger(const LADSPA_Descriptor * Descriptor,
	...
	if ((ptr = (void **) malloc(sizeof(ChorusFlanger))) != NULL) {
...
//!!! Modified.  No filter, wet/dry, delay has feedback to 
// make a proper flanging effect
...
run_ChorusFlanger(LADSPA_Handle Instance,
   ...
   unsigned long readPosL;
   unsigned long readPosR;
	...
	for (sample_index = 0; sample_index < sample_count; sample_index++) {
      ...
      readPosL++;
      readPosR++;
      if (readPosL > ptr->buflen_L)
         readPosL = 0;
      if (readPosR > ptr->buflen_R)
         readPosR = 0;
		...
		s_a_L = read_buffer(ptr->ring_L, ptr->buflen_L,
				    readPosL, (unsigned long) n_L);
		s_b_L = read_buffer(ptr->ring_L, ptr->buflen_L,
				    readPosL, (unsigned long) n_L + 1);
		
		s_a_R = read_buffer(ptr->ring_R, ptr->buflen_R,
				   readPosR, (unsigned long) n_R);
		s_b_R = read_buffer(ptr->ring_R, ptr->buflen_R,
				    readPosR, (unsigned long) n_R + 1);
		
		d_L = ((1 - rem_L) * s_a_L + rem_L * s_b_L);
		d_R = ((1 - rem_R) * s_a_R + rem_R * s_b_R);

      push_buffer(in_L + d_L * 0.8, ptr->ring_L, ptr->buflen_L, &(ptr->pos_L));
		push_buffer(in_R + d_R * 0.8, ptr->ring_R, ptr->buflen_R, &(ptr->pos_R));
...
_init() {
	...
	stereo_descriptor->run_adding = NULL; /*run_adding_ChorusFlanger;*/
	stereo_descriptor->set_run_adding_gain = NULL; /*set_run_adding_gain_ChorusFlanger;*/
